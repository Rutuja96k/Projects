...
void man(char *name)
{
    ...
    {
     printf("Description : Used to opened file\n");
...
{
    if(((UFDTArr[fd].ptrfiletable->ptrinode->mode)!= WRITE) &&((UFDTArr[fd].ptrfiletable->ptrinode->mode)!=READ+WRITE))  return -1;
...
int LseekFile(int fd, int size, int from)
{
    ...
    else if(UFDTArr[fd].ptrfiletable->mode==WRITE)
    {
        ...
        {
            if((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)+size > MAXFILESIZE) return -1;
            if(((UFDTArr[fd].ptrfiletable->ptrinode->writeoffset)+size)<0) return -1;
            (UFDTArr[fd].ptrfiletable->ptrinode->writeoffset) =(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) +size;
        }
    }
}
